...
var sqldbProvision = function (log, params) {
    ...
    this.fixupParameters = function () {
        ...
        if (_.isUndefined(params.parameters.sqlServerParameters.properties.transparentDataEncryption)) {
            //setup default value for TDE parameter
            params.parameters.sqlServerParameters.properties.transparentDataEncryption = 'Disabled';
        }
    ...
    this.provision = function (sqldbOperations, resourceGroup, next) {
        ...
        if (!privilege.allowToCreateSqlServer) {
            ...
            function(result, callback) { // Set transparent Data encryption if requested
                var tde = params.parameters.sqlServerParameters.properties.transparentDataEncryption;
                log.info('sqldb cmd-provision: set transparent data protectection');
                //log.info('Callback is %j', callback);
                if (tde === 'Disabled'){
                    callback(null, result);
                }
                sqldbOperations.setTransparentDataEncryption(tde, function (err, result) {
                    if (err) {
                        log.error('sqldb cmd-provision: setTransparentDataEncryption: err: %j', err);
                        callback(err);
                    } else if ((result.statusCode === HttpStatus.OK) || (result.statusCode === HttpStatus.CREATED)) {
                        log.info('sqldb cmd-provision: setTransparentDataEncryption: success');
                        callback(null, result);
                    } else {
                        log.error('sqldb cmd-provision: setTransparentDataEncryption, unexpected error: result: %j', result);
                        callback(Error(result.body.message));
                }
            ], function (err, result) {
                if (err) {
                    log.info('sqldb cmd-provision: final callback: err: ', err);
