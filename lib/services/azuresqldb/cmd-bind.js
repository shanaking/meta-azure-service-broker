    ...
    this.bind = function (sqldbOperations, next) {
        ...
        async.waterfall([
            ...
            function(callback) {
                    ...
                    log.info('sqldb cmd-bind: set transparent data protecction');
                    ...
                    sqldbOperations.setTransparentDataEncryption(tde, function (err, result) {
                        if (err) {
                            log.error('sqldb cmd-bind: setTransparentDataEncryption: err: %j', err);
                            callback(err);
                        } else if ((result.statusCode === HttpStatus.OK) || (result.statusCode === HttpStatus.CREATED)) {
                            log.info('sqldb cmd-bind: setTransparentDataEncryption: success');
                            callback(null, result);
                        } else {
                            log.error('sqldb cmd-bind: setTransparentDataEncryption, unexpected error: result: %j', result);
