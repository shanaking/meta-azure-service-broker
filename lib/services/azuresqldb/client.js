...
// Set transparent data encryption newStatus is [Enabled|Disabled]
sqldbOperations.prototype.setTransparentDataEncryption = function (newStatus, callback){
    ...
    that.PUT(that.transparentDataEncryptionUrl, headers, data, API_VERSIONS.SQL, function(err, res, body) {
        ...
        if (err) {
            that.log.info('sqldb client: set transparent data encryption: err %j', err);
            callback(err, null);
        } else if ((res.statusCode === HttpStatus.OK) || (result.statusCode === HttpStatus.CREATED)) {
            callback(null, result);
        } else {
            that.log.info('sqldb client: set transparent data encryption: body: %j', body);
            result.body = body;
            callback(null, result);
        }
...
sqldbOperations.prototype.createServer = function (parameters, callback) {
    ...
    that.log.debug('sqldb client: createServer: parameter.properties %j', parameters.sqlServerParameters.properties);
...
sqldbOperations.prototype.createDatabase = function (parameters, callback) {
    ...
    that.log.info('sqldb client - createDatabase sqldbparameters: %j', parameters.sqldbParameters);
